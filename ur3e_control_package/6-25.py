...
def main():
    ...
    joint_trajectory_pub = node.create_publisher(JointTrajectory, '/scaled_joint_trajectory_controller/joint_trajectory', QoSProfile(depth=10))
    ...
    def move_to_waypoints():
        ...
        for idx, waypoint in enumerate(WAYPOINTS):
            ...
            if idx == 0:
                # Calculate the distance from the current position to the first waypoint
                distance = np.linalg.norm(np.array(current_position) - np.array(waypoint_position))
            else:
                distance = np.linalg.norm(np.array(waypoint_position) - np.array(WAYPOINTS[idx-1][0]))
            ...
            node.get_logger().info(f"Moving to waypoint {idx + 1}")
